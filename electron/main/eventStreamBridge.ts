...
import { formatEventStreamLine } from '../../src/lib/eventStreamFormat'
import {
...
function handleIncomingText(raw: string) {
  ...
  if (getEventStreamLogSession()) {
    const formatted = formatEventStreamLine(event)
...
export function registerEventStreamBridge(getWins: () => BrowserWindow[]) {
  ...
  ipcMain.handle(
    ...
    async (_e, payload: unknown): Promise<{ ok: true } | { ok: false; error: string }> => {
      ...
      const enableLog = o.logging !== false
      ...
      try {
        ...
        if (enableLog) beginEventStreamLogSession()
